...
const SideBar = ({ openProfile, setOpenProfile, reload, setReload }) => {
  ...
  return (
    <div className="flex items-center justify-between w-full bg-zinc-800 p-2 relative">
      <div className="flex items-center">
        ...
        />
        <h1 className="text-md font-semibold text-purple-200 italic select-none">
          {" "}
          Swift Chat
        </h1>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        >
          <i className="ri-search-2-line text-lg group-hover:text-purple-400"></i>
          <input
            className="bg-transparent px-3 outline-none hidden md:flex placeholder-zinc-400 text-sm cursor-pointer m-0"
            type="text"
            placeholder="Search User"
            onClick={() => setOpenSearch(!OpenSearch)}
            readOnly
                  ...
                  <div className="p-2 space-y-2">
                    {notifications.map((notif) => (
                      ...
                      >
                        <div className="flex items-start space-x-3">
                          <div className="flex-shrink-0">
                            <div className="w-8 h208 bg-purple-500 rounded-full flex items-center justify-center">
                              <i className="ri-message-3-line text-white"></i>
                            </div>
                          </div>
                          <div className="flex-1 min-w-0">
        ...
        <div className="relative">
          ...
          >
            <img
              src={user?.picture || "default_profile_picture.png"}
              alt={user?.name}
              className="h-8 w-8 rounded-full object-cover"
            />
          </button>
          ...
          {isOpen && (
            <div className="absolute right-0 mt-3 w-48 bg-zinc-800 rounded-md shadow-lg shadow-purple-500/20 p-1 flex flex-col gap-1">
              <div
                ...
                className="px-4 py-2 text-sm text-zinc-200 bg-zinc-700 hover:bg-purple-600 hover:text-white cursor-pointer rounded-md transition-colors duration-200"
              >
                {user?.name}
              </div>
              <div
